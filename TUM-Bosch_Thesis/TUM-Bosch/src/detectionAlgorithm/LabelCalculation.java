...
public class LabelCalculation {
	public static void calculateLabelBasedonActTemp(String filepath,
		...
		int[] cpr = new int[uniquedate_l]; // circulation_pump_request
		...
		if (uniquedate_l != 0) {
			...
			for (int i = 0; i < line.split(",").length; i++) {
				...
				if (act_temp.contains("circulation_pump_request")) {
			...
			while (((line = br.readLine()) != null) && (i < timecount)) {
				...
				for (j = 0; j < at.length; j++) {
					...
					String cp_req = line.split(",")[at[j] + 4];
					weekday[j][i] = day;
			...
			for (j = 0; j < at.length; j++) {
				for (k = 0; k < timecount; k++) {
					...
					if (j > 0 && k < (timecount)
							&& (diff[j][k]) >= median[j - 1]) {
						label[j][k] = "USAGE";
					} else if (j == 0 && k < (timecount) && (diff[j][k]) >= 0.6) {
						label[j][k] = "USAGE";
					}
				}
			}
