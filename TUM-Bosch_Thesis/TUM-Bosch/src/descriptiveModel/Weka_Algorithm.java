...
public class Weka_Algorithm {
	...
	public static double applyWeka_KStar(String detection_filepath,
		...
		String prediction_output = final_output.split("_Actual.csv")[0]
				+ "__prediction.csv";
		BufferedWriter bw = new BufferedWriter(
				new FileWriter(prediction_output));
		BufferedReader br = new BufferedReader(new FileReader(final_output));
		...
		for (int k = 0; k < test.numInstances(); k++) {
			double pred = imc.classifyInstance(test.instance(k));
			// int id = (int) Math.round(test.instance(k).value(1));
			String[] params = br.readLine().split("\\,");
			int ci = test.classAttribute().numValues();
			if (ci <= pred) {
				pred = pred - 1;
			}
			String class_label = test.classAttribute().value((int) pred);
			bw.write(params[0] + "," + params[1] + "," + params[2] + ","
					+ params[3] + "," + params[4] + "," + class_label + "\n");
		}
		bw.flush();
		bw.close();
		br.close();
	...
	public static void applyWeka_Vote(String detection_filepath,
		...
		/*
		 * options = new String[2]; options[0] = "-R"; options[1] = "3"; remove
		 * = new Remove(); remove.setOptions(options);
		 * remove.setInputFormat(test); Instances newTest =
