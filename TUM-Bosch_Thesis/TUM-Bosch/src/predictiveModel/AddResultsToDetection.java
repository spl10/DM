...
public class AddResultsToDetection {
			...
			for (int i = li_dsm.size() - 1; i >= 0; i--) {
				...
				if (f.exists()) {
					...
					if (emptyFile) {
						...
						month = preventRewriting_days(new_dates,
								detection_filepath, filepath);

						// Creates prediction file for the last date in the
						// last
						// month
						// in the list of files.
						if ((month != null) && (month.equals(config_month)))
	 ...
	 * @throws Exception
	 */
	public static String preventRewriting_days(String[] new_dates,
		...
		String month = filepath.split("output")[0].substring(
				filepath.split("output")[0].lastIndexOf("\\") - 2).replace(
				"\\", "");
		BufferedWriter bw = new BufferedWriter(new FileWriter(
				detection_filepath, true));
		int i = 0;
