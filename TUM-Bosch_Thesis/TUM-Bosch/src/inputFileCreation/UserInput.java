...
import predictiveModel.AddResultsToDetection;

/**
 * Description: Class with main() function. This class traverse through the
 * folder hierarchy and calls EMSConverter tool which converts the raw binary
 * files to csv format.
 * 
 */
public class UserInput {
	public static void main(String[] args) throws Exception {
		...
		String file_suffix = "_NEW_" + timestep + "ts_vis.txt";
		...
		for (int i = gateways.length - 1; i >= 0; i--) {
			if (gateways[i].isDirectory()) {
				...
				while (it.hasNext()) {
					...
					f = new File(filepath);
					if (!f.exists()) {
						ems_2_csv_ExternalPackage.gui.EMSConverter ems = new ems_2_csv_ExternalPackage.gui.EMSConverter();
				...
				dates = detectionDates(li_dsm, config_month, config_year);
	...
	/**
	 * Description: Returns a list of dates to be added to the training
	 * data-set.
	 * 
	 * @param li_dsm
	 * @param config_month
	 ...
	 * @throws Exception
	 */
	public static String[] detectionDates(List<String> li_dsm,
